...
Later we will need the  _cumulative distribution function_ wchih is simply deinfed by 
...
The pdf is often shown as a table or a grpah. For the graph we simply plot a bar for each $k$ going up to $p_k$.
...
mean and stadnard deviation of $X$.
...
* We define the variance because mathematical calculations are easier without the square root. Other than that we always use standard devatiion.

* The standard deviation has the same units as $X$. The variance has units squared which has no interpretation. What is kilograms sauared or dollars squared?
:::
...
* $X$ is either 1 (success) or failiure (0). 
...
Bernoulli trias are popular because we can use them to count random things: number of heads when we toss coins, number of lottery winners, number of defective light bulbs made in a day by a factory, number of patients that got cured by a drug, number of COVID-19 hospitalizations in a day, and so on.
...
If $X$ and $Y$ are two random varialbes, what is $\mu_{X-Y}$
...
* **If $X$ and $Y$** are independnet, $\sigma^2_{X+Y} = \sigma_X^2 + \sigma_Y^2 \implies \sigma_{X+Y} = \sqrt{\sigma_X^2 + \sigma_Y^2}$:
...
If $X$ and $Y$ are two **independent** random varialbes, what is $\sigma_{X-Y}$
...
Interpretation: Subtracting two variables that vary independenty has more variability than each.
:::
...
It is also common ask how many trials do I need to see a success. For example, how many free throws will Curry take until he misses.
...
Define a random variable as $X$=number of trials we see before the first success. 
...
Here is a $n=10, p=0.5$  binomial with a normal with mean $np = 5$ and standard devataion $\sqrt{np(1-p)} \approx 1.6$ added in blue
...
If I toss 100 coins, what is the probability that I see between than 45 and 55 heads?
